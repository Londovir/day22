...
import { Cube, Instruction, interval } from './types.js';
...
function GetResult(data: Instruction[]) {
     const litcubes = [] as Cube[];
     ...
     const litcount = litcubes.reduce(
          (p, c) => p + (Math.abs(c.x[1] - c.x[0]) + Math.abs(c.y[1] - c.y[0]) + Math.abs(c.z[1] - c.z[0]) + 3),
          0
     );
     return litcount;
}

function GetResultPart2(data: any) {}

function SubtractCubes(cube1: Cube, cube2: Cube) {
     ...
     if (xoverlaps == null && yoverlaps == null && zoverlaps == null) return [];
...
function GetDisjoints(range1: [number, number], range2: [number, number]): interval[] | null {
     // If range2 does not overlap range1 at all, return null. (No disjoints needed)
     if (!(range2[1] >= range1[0] && range2[0] <= range1[1])) return null;
     ...
     if (range2[0] <= range1[0] && range2[1] >= range1[0] && range2[1] < range1[1]) {
          ...
          return [[range2[1] + 1, range1[1]]] as interval[];
...
function ReduceCubes(cubes: Cube[]) {
